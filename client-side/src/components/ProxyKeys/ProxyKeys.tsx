...
const ProxyKeys: FC<Props> = ({ onLock }) => {
  ...
  useEffect(function loadKeys() {
    ...
    (async () => {
      ...
      setKeys(keys);
      setLoading(false);
      // todo setErrorMessage((await response.json()).error_message || 'some error');
    })();
  }, []);
  ...
  async function fetchAllKeys() {
    ...
    // todo throw error with error_message
  }
  ...
  async function createKey(event: React.FormEvent) {
    ...
    await fetch(app.serverBaseUrl + '/api/keys', {
      credentials: 'include',
      method: 'post',
